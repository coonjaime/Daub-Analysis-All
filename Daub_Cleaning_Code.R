...
VegData<-read_csv("DaubData_WithMetadata_2007-2018,2021-2023_JJC_9.22.23.csv")%>%
  ...
  mutate(CSG_FEAR     = if_else(Year < 2022, NA_character_, CSG_FEAR))%>%
  mutate(Litter_depth = if_else(Litter_depth > 80, NA_character_, Litter_depth))%>%
  ...
  mutate(Sedges       = fct_recode(Sedges, NULL = "."),
         ...
         Seed_mix     = fct_recode(Seed_mix, NULL = "."),
         Seed_mix     = as.numeric(as.character(Seed_mix)),
         
         Litter_depth = fct_recode(Litter_depth, NULL = "."),
         Litter_depth = as.numeric(as.character(Litter_depth)))%>%
  #filter(Year>2007)%>%
  clean_names()%>%
  mutate(litter       = ifelse(year <  2012,NA_real_,litter))%>% #this replaces zeros with NAs for years we did not collect data
  mutate(bare         = ifelse(year <  2012,NA_real_,litter))%>% #this replaces zeros with NAs for years we did not collect data
  mutate(litter_depth = ifelse(year <  2012,NA_real_,litter_depth))%>%
  mutate(sedges       = ifelse(year >  2016,NA_real_,sedges))%>%
  mutate(panic        = ifelse(year <  2014,NA_real_,panic))%>%
  mutate(seed_cover   = ifelse(year <  2017,NA_real_,seed_cover))%>%
  mutate(seed_mix     = ifelse(year <  2014,NA_real_,seed_mix))%>%
...
Veg_Avg_Data_Year = VegData %>%
  group_by(year)%>%
  summarise_at(vars(litter,litter_depth:woody,panic:sdv,avg_robel,sd_robel),list(mean="mean",sd="sd"), na.rm=TRUE)
Veg_Avg_Data_Year
